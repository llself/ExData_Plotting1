...
#Leslie Self
#December 4, 2014
#Plot4.R - creates a plot  - 3 line charts to show the different sub metering values in reference to Date/Time
...
#create the four plots...using mfrow so order of plots is topleft, top right, bottom left, bottom right
par(mfrow = c(2,2), mar = c(4,4,2,1))
with(powerDS, {
  plot(powerDS[, 10], powerDS[,3], type="l", xlab="", ylab="Global Active Power (kilowatts)")
  plot(powerDS[, 10], powerDS[,5], type = "l", xlab = "datetime", ylab="Voltage")
  plot(powerDS[, 10], powerDS[,7], ylab="Energy sub metering", xlab="",  type = "l")
  ...
  plot(powerDS[, 10], powerDS[,4], type = "l", xlab = "datetime", ylab="Global_reactive_power")
})
